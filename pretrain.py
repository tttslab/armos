...
from env.utils import Batch_generator
import configparser
import time
import logging
import librosa
...
# TODO: 他のモデルとの共通部分をまとめる
logging.basicConfig(filename=os.path.join('./log', 'dpg_pn', 'pretrain.log'), level=logging.INFO)

print(os.path.join("./config", "dpg_pn", "config.ini"))
### Hyperparameters
conf = configparser.SafeConfigParser()
conf.read(os.path.join("./config", "dpg_pn", "config.ini"))
...
policy       = networks.stacked_BLSTM(IN_SIZE, OUT_SIZE, P_HIDDEN_SIZE, P_NUM_LAYERS, 0).to(device)
loss_fun     = nn.MSELoss()
p_optim      = torch.optim.Adam(policy.parameters(), lr=1e-3)
train_loader = Batch_generator('training', BATCH_SIZE)
...
for iteration in range(100):
    ...
    logging.info('iter: {}\tloss: {}\telapse: {:.02f}'.format(iteration+1,\
                                                              loss.item(),\
                                                              time.time()-start))
torch.save(policy.state_dict(),  os.path.join('./exp', 'dpg_pn', 'pretrain.model'))
