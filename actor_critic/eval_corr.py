...
rmse = 0
min_val = np.asarray([0.0, -3.228, -0.372, -7.0, -1.0, -1.102, 0.0, -0.1, 0.0, -3.194, -1.574, 0.873, \
                      -1.574, -3.194, -1.574, -4.259, -3.228, -1.081, -1.081, -1.081, -1.081, 0.0, 0.0, 0.0])
max_val = np.asarray([1.0, -1.85, 0.0, 0.0, 1.0, 2.205, 1.0, 1.0, 1.0, 2.327, 0.63, 3.2, \
...
with torch.no_grad():
    ...
    target = target * (max_val - min_val) + min_val
    corr   = np.zeros((101, 24))
    for i in range(101):
        for j in range(24):
            corr[i,j] = np.corrcoef(action[i,:,j], target[i,:,j])[0,1]
    corr = np.asarray([corr.mean(axis=0)])
    with open('log/eval_corr.csv', 'a') as f:
