...
for iteration in range(100):
    ...
    m_target = torch.ones(BATCH_SIZE, AUDIO_SEGMENT, OUT_SIZE).to(device) * 0.5
    v_target = torch.ones(BATCH_SIZE, AUDIO_SEGMENT, OUT_SIZE).to(device) * 0.1

    ### Get action
    mean, var = policy(inputs, length.tolist())
    loss      = F.mse_loss(mean, m_target) + F.mse_loss(var, v_target)
    if iteration % 10 == 0:
        x=1+1
    p_optim.zero_grad()
    loss.backward()
