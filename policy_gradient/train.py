...
for iteration in range(10000000):
    ...
    reward_mean = 0
    reward_tensor = torch.zeros(BATCH_SIZE, 1).to(device)
    for i in range(BATCH_SIZE):
        reward_tensor[i] = F.mse_loss(torch.from_numpy(reward[i][:length[i]]).to(device), inputs[i, :length[i]], reduction='none').mean()
    ...
    logging.info('iter: {}\treward: {}\telapse: {:.02f}'.format(iteration+1,\
                                                                reward_mean,\
                                                                time.time()-start))
