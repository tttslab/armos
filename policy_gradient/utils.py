...
def synthesis(tractParams, glottisParams, duration, queue):
    synthesized = TractSeqToWave_Example.vtlSynthesize(tractParams, glottisParams, duration+frameRate_Hz/1000.0)
    ...
    if FEAT_TYPE == 'mfcc':
        ...
        rec_feats   = librosa.feature.melspectrogram(synthesized[int(frameRate_Hz/2000.0*16000)-1:int((frameRate_Hz/2000.0+0.05)*16000)],\
                                                     sr=SAMPLING_RATE,\
                                                     n_mels=IN_SIZE,\
                                                     n_fft=int(SAMPLING_RATE/40.0),\
                                                     hop_length=int(1.0*SAMPLING_RATE/frameRate_Hz))
...
def Batch_generator(dataset, batch_size):
    if dataset == 'training':
        datalist_txt = open('datalist/training_list.txt', 'r')
